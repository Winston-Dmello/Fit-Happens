...
class Pose:
    ...
    def get_point(self, str_point):
        ...
        return (
            self.key_points[POSE[str_point]]
    ...
    def predict_pose(self):
        ...
        is_lean_left = is_lean_right = is_pushup = is_plank = is_squat = is_jumping_jack = False
        diff_head_hand_y = None

        if self.is_point_in_keypoints("left_shoulder") and self.is_point_in_keypoints("right_shoulder"):
            sh_angle = self.one_line_angle("left_shoulder", "right_shoulder")
            if sh_angle<170 and sh_angle>120:
                is_lean_right = True
            elif sh_angle<-140 and sh_angle>-170:
                is_lean_left = True

        # Calculate angle between lines shoulder-elbow, elbow-wrist
        if (
            self.is_point_in_keypoints("left_shoulder")
            and self.is_point_in_keypoints("left_elbow")
            and self.is_point_in_keypoints("left_wrist")
        ):
            ang1 = self.two_line_angle("left_shoulder", "left_elbow", "left_wrist")
        elif (
            self.is_point_in_keypoints("right_shoulder")
            and self.is_point_in_keypoints("right_elbow")
            and self.is_point_in_keypoints("right_wrist")
        ):
        ...
        if (
            self.is_point_in_keypoints("left_shoulder")
            and self.is_point_in_keypoints("left_hip")
            and self.is_point_in_keypoints("left_ankle")
        ):
            ang2 = self.two_line_angle("left_shoulder", "left_hip", "left_ankle")
        elif (
            self.is_point_in_keypoints("right_shoulder")
            and self.is_point_in_keypoints("right_hip")
            and self.is_point_in_keypoints("right_ankle")
        ):
        ...
        left_shoulder_ankle = self.is_point_in_keypoints(
            "left_shoulder"
        ) and self.is_point_in_keypoints("left_ankle")
        right_shoulder_ankle = self.is_point_in_keypoints(
            "right_shoulder"
        ) and self.is_point_in_keypoints("right_ankle")
        left_hip_ankle = self.is_point_in_keypoints(
            "left_hip"
        ) and self.is_point_in_keypoints("left_ankle")
        right_hip_ankle = self.is_point_in_keypoints(
            "right_hip"
        ) and self.is_point_in_keypoints("right_ankle")
        ...
        left_elbow_wrist = self.is_point_in_keypoints(
            "left_elbow"
        ) and self.is_point_in_keypoints("left_wrist")
        right_elbow_wrist = self.is_point_in_keypoints(
            "right_elbow"
        ) and self.is_point_in_keypoints("right_wrist")
        ...
        left_knee_ankle = self.is_point_in_keypoints(
            "left_knee"
        ) and self.is_point_in_keypoints("left_ankle")
        right_knee_ankle = self.is_point_in_keypoints(
            "right_knee"
        ) and self.is_point_in_keypoints("right_ankle")
        ...
        left_hip_knee = self.is_point_in_keypoints(
            "left_hip"
        ) and self.is_point_in_keypoints("left_knee")
        right_hip_knee = self.is_point_in_keypoints(
            "right_hip"
        ) and self.is_point_in_keypoints("right_knee")
        ...
        if (
            self.pushup_counter >= 24
            and len(self.ang1_tracker) == 24
            and len(self.ang4_tracker) == 24
        ):
        ...
        # Distance algorithm
        head_point = self.get_available_point(
        ...
        hand_point = self.get_available_point(
            [
                "left_wrist",
                "right_wrist",
                "left_pinky",
        ...
        if ang3 is not None and ang5 is not None and diff_head_hand_y is not None:
            if ((70 <= ang3 <= 110) or (70 <= ang5 <= 110)) and len(
                self.headpoint_tracker
            ) == 24:
                height_mean = int(
                    sum(self.headpoint_tracker) / len(self.headpoint_tracker)
                )
                height_norm = self.operation.normalize(
                    height_mean, head_point[1], foot[1]
                )
                del self.headpoint_tracker[0]
                if (
                    height_norm < 0
                    and diff_head_hand_y < 0
                    and not 70 <= abs(ang6) <= 110
                ):
                    is_squat = True
    ...
    def estimate(self) -> None:
        ...
        out = cv2.VideoWriter(
            "output.avi", self.fourcc, self.video_fps, (self.width, self.height)
        )
        while self.video_reader.is_opened():
            ...
            if results.pose_landmarks is not None:
                ...
                if estimated_pose is None:
                    ...
                    if (
                        len(self.pose_tracker) == 10
                        and len(set(self.pose_tracker[-6:])) == 1
                    ):
                        image = self.draw.pose_text(
                            image, "Prediction: " + estimated_pose
                        )
                        if estimated_pose != self.prev_pose:
                            ...
                            try:
                                ...
                                self.prev_pose = estimated_pose

                            except requests.exceptions.RequestException as e:
                                print(f"Couldn't send pose: {e}")
...
class Pushup(Pose):
    ...
    def _draw(self, image):
        ...
        left_shoulder_wrist_foot = (
            self.is_point_in_keypoints("left_shoulder")
            and self.is_point_in_keypoints("left_wrist")
            and self.is_point_in_keypoints("left_foot_index")
        )
        right_shoulder_wrist_foot = (
            self.is_point_in_keypoints("right_shoulder")
            and self.is_point_in_keypoints("right_wrist")
            and self.is_point_in_keypoints("right_foot_index")
        )
        if left_shoulder_wrist_foot:
            image = self.draw.draw_line(
                image, self.get_point("left_shoulder"), self.get_point("left_wrist")
            )
            image = self.draw.draw_line(
                image, self.get_point("left_wrist"), self.get_point("left_foot_index")
            )
            image = self.draw.draw_line(
                image,
                self.get_point("left_foot_index"),
                self.get_point("left_shoulder"),
            )
        elif right_shoulder_wrist_foot:
            image = self.draw.draw_line(
                image, self.get_point("right_shoulder"), self.get_point("right_wrist")
            )
            image = self.draw.draw_line(
                image, self.get_point("right_wrist"), self.get_point("right_foot_index")
            )
            image = self.draw.draw_line(
                image,
                self.get_point("right_foot_index"),
                self.get_point("right_shoulder"),
            )
        else:
            pass
        return image
    ...
    def pose_algorithm(self):
        ...
        # Angle algorithm
        head_pos = self.operation.point_position(
            head_point, (self.width / 2, 0), (self.width / 2, self.height)
        )
        wrist = self.get_available_point(["left_wrist", "right_wrist"])
        ang = self.operation.angle(head_point, ankle, wrist)
        if (
            diff_y < 250
            and (ang < 40 and head_pos == "right")
            or (ang > 140 and head_pos == "left")
        ):
            self.is_pushup = True
    ...
    def measure(self) -> None:
        ...
        while self.video_reader.is_opened():
            ...
            image = cv2.rectangle(
                image,
                (0, self.height // 8 - 10),
                (self.width // 10 * progress_counter, self.height // 8),
                progress_bar_color,
                cv2.FILLED,
            )
            if results.pose_landmarks is not None:
                ...
                image = self.draw.pose_text(
                    image, "Pushups Count: " + str(self.pushups_count)
                )
                pushup_count_prev = pushup_count_current
                pushup_count_current = self.pushups_count
                if (
                    self.pushups_count > 0
                    and abs(pushup_count_current - pushup_count_prev) == 1
                ):
                    progress_counter += 1
...
class Plank(Pose):
    ...
    def _draw(self, image):
        ...
        if left_shoulder_wrist_foot:
            image = self.draw.draw_line(
                image, self.get_point("left_shoulder"), self.get_point("left_elbow")
            )
            image = self.draw.draw_line(
                image, self.get_point("left_elbow"), self.get_point("left_foot_index")
            )
            image = self.draw.draw_line(
                image,
                self.get_point("left_foot_index"),
            ...
            image = self.draw.draw_line(
                image, self.get_point("right_shoulder"), self.get_point("right_elbow")
            )
            image = self.draw.draw_line(
                image, self.get_point("right_elbow"), self.get_point("right_foot_index")
            )
            image = self.draw.draw_line(
                image,
                self.get_point("right_foot_index"),
    ...
    def measure(self) -> None:
        ...
        while self.video_reader.is_opened():
            ...
            image = cv2.rectangle(
                image,
                (0, self.height // 8 - 10),
                (self.width // 60 * progress_counter, self.height // 8),
                progress_bar_color,
                cv2.FILLED,
            )
            if results.pose_landmarks is not None:
                ...
                image = self._draw(image)
                time = round(
                    time_adjustment * (self.timer.get_current_time() + self.total_time),
                    2,
                )
                h_m_s_ms_time = self.timer.convert_time(
                    time
                )  # convert Seconds to Hour : Minute : Second : Milli-Second format
                image = self.draw.pose_text(
                    image, "Plank Timer: " + str(h_m_s_ms_time)[:10]
                )
    ...
    def measure(self) -> None:
        ...
        while self.video_reader.is_opened():
            ...
            if results.pose_landmarks is not None:
                ...
                self.pose_algorithm()
                image = self.draw.pose_text(
                    image, "Squats Count: " + str(self.squats_count)
                )
                squads_count_prev = squad_count_current
                squad_count_current = self.squats_count
                if (
                    self.squats_count > 0
                    and abs(squad_count_current - squads_count_prev) == 1
                ):
                    progress_counter += 1
                    if progress_counter == 10:
                        progress_counter = 0
    ...
    def measure(self) -> None:
        ...
        while self.video_reader.is_opened():
            ...
            if results.pose_landmarks is not None:
                ...
                self.pose_algorithm()
                image = self.draw.pose_text(
                    image, "JumpingJacks Count: " + str(self.jumping_jack_count)
                )
                jj_count_prev = jj_count_current
                jj_count_current = self.jumping_jack_count
                if (
                    self.jumping_jack_count > 0
                    and abs(jj_count_current - jj_count_prev) == 1
                ):
                    progress_counter += 1
                    if progress_counter == 10:
                        progress_counter = 0
